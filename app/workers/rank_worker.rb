class RankWorker
  ...
  def perform(user_id)
    ...
    #user.repositories.with_language.select("language, count(repositories.id) as repository_count, sum(stars) as stars_count").group("repositories.language").each do |repos|
